...
def calculate_intersection_points():
    # [Previous intersection points calculation remains the same]
    points = [
        # Center
        [0, 0, 0],
        # Square corners
...
def create_helicon_geometry():
    ...
    # Single isosceles triangle (bottom to top)
    triangle_points = np.array([
        [-1, -1, 0],  # Left base point
        [1, -1, 0],   # Right base point
        [0, 1, 0]     # Apex (midpoint of opposite side)
    ])
    ...
    triangle_lines.paint_uniform_color([0.4, 0.4, 0.8])  # Blue color
...
def visualize_helicon():
    ...
    vis.create_window(window_name="Helicon Geometry with Test Triangle")
