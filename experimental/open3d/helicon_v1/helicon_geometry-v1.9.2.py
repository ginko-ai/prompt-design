...
def calculate_triangle_semicircle_intersections():
    # Extended structure to include type of intersection and ID
    intersections = [
...
def create_text_label(point, text):
    # This function would be used if we want to add visible IDs
    # Currently Open3D doesn't support text rendering directly
    # We could potentially use other methods to visualize IDs
    pass

def create_helicon_geometry():
    geometries = []

    # [Previous geometry creation code remains the same until intersection points]

    # Create intersection points with type information
    ...
    for point in intersection_data:
        ...
        # If we implement text labels:
        # label = create_text_label(point, f"ID: {int(point[4])}")
        # if label:
